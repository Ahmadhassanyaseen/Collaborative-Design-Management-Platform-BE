...
const BlogService = {
  async createPost(req) {
    ...
    const finalSlug = slug || name.toLowerCase().replace(/[^a-zA-Z0-9]+/g, '-').replace(/^-+|-+$/g, '');
  ...
  async getAllPostsByWebsite(websiteLink) {
    ...
    const website = await BlogWebsiteModel.findOne({ link: websiteLink, isDeleted: false });
    if (!website) {
    ...
    const allPosts = await PostModel.find({ isDeleted: false })
      .select('websites status');
  ...
  async updatePostImage(id, req) {
   const updateData = { featuredImage: req.imageUrl };
    ...
    return await PostModel.findByIdAndUpdate(id, updateData, { new: true }).populate("category").populate("websites").populate("createdBy");
  },
